...
function dimensionChange(e,dimname){
	...
	if(img2imgMode=='图转图' && redrawImage){
		targetElement = redrawImage;
	}else if(img2imgMode=='内补绘制' && inpaintImage){
		...
		inputs.forEach(function(e){ 
			...
			if(parentLabel && parentLabel.innerText){
				if(!e.classList.contains('scrollwatch')){
					if(parentLabel.innerText == '宽度' || parentLabel.innerText == '高度'){
						e.addEventListener('input', function(e){dimensionChange(e,parentLabel.innerText)} )
						e.classList.add('scrollwatch')
					}
					if(parentLabel.innerText == '宽度'){
						currentWidth = e.value*1.0
					}
					if(parentLabel.innerText == '高度'){
